...
from custom_components.heating_control.models import DeviceDecision, GasHeaterDecision
from custom_components.heating_control.const import DEFAULT_FINAL_SETTLE, DEFAULT_SETTLE_SECONDS, DEFAULT_SCHEDULE_FAN_MODE
...
async def test_turn_on_device_with_full_update(dummy_hass: DummyHass, no_sleep):
    ...
    await controller.async_apply([_decision("climate.living_room", on=True, temp=23.5, fan="high")], None)
...
async def test_turn_off_device(dummy_hass: DummyHass, no_sleep):
    ...
    await controller.async_apply([_decision("climate.bedroom", on=True, temp=20.0, fan="auto")], None)
    dummy_hass.services.calls.clear()

    await controller.async_apply([_decision("climate.bedroom", on=False, temp=20.0, fan="auto")], None)
...
async def test_no_changes_no_calls(dummy_hass: DummyHass, no_sleep):
    ...
    await controller.async_apply([decision], None)
    dummy_hass.services.calls.clear()
...
async def test_fan_mode_not_supported(dummy_hass: DummyHass, no_sleep):
    ...
    await controller.async_apply([_decision("climate.study", on=True, temp=21.0, fan="high")], None)
...
@pytest.mark.asyncio
async def test_gas_heater_wrapped_as_device(dummy_hass: DummyHass, no_sleep):
    dummy_hass.states.set(
        "climate.gas",
        DummyState("off", {"fan_modes": ["auto"]}),
    )
    ...
    gas_decision = GasHeaterDecision(
        entity_id="climate.gas",
        should_be_active=True,
        target_temp=25.0,
        target_fan=DEFAULT_SCHEDULE_FAN_MODE,
        active_schedules=("Boost",),
    )

    await controller.async_apply([], gas_decision)

    services = [call["service"] for call in dummy_hass.services.calls]
    assert services.count("set_hvac_mode") == 1
    assert services.count("set_temperature") == 1
